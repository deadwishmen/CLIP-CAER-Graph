...
from dataloader.video_dataloader import train_data_loader, test_data_loader, custom_collate_fn
...
def build_model(args: argparse.Namespace, input_text: list) -> torch.nn.Module:
    ...
    trainable_params_keywords = ["image_encoder", "temporal_net", "gnn_encoder", "prompt_learner", "temporal_net_body", "project_fc"]
...
def build_dataloaders(args: argparse.Namespace) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]: 
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers, pin_memory=True, drop_last=True, collate_fn=custom_collate_fn
    )
    val_loader = torch.utils.data.DataLoader(
        test_data, batch_size=args.batch_size, shuffle=False,
        num_workers=args.workers, pin_memory=True, collate_fn=custom_collate_fn
    )
